def calc():
    ...
    line = input('Введите выражение в формате <число> <операция> <число>: ')
    
    #Убираю лишниее пробелы в начале и уонце строки
    line = line.strip()
    
    #Создаю словарь для перевода самостоятельных значений
    dict = {'один' : '1' , 'два' : '2', 'три' : '3', 'четыре' : '4', 'пять' : '5', 'шесть' : '6',
    ...
    for part in range(0, len(listl)):
        ...
        if listl[part] in dict:
            ...
            if dict[listl[part]].isnumeric():
                ...
                #Проверка: Если мы рассматриваем не последний элемент списка listline
                    ...
                    if listl[part + 1] in dict:
                        ...
                        if dict[listl[part + 1]].isnumeric():
                            #То к результирующей переменной являющейся строкой прибавляем предыдущее и следующее строковое значние из словаря
                            result += str(int(dict[listl[part]]) + int(dict[listl[part+1]]))
                        else:
                            #Иначе прибавляем текущий элемент словаря
                            result += dict[listl[part]]
                    else:
                        #Иначе: неверный ввод (еще одна защита от дурака
